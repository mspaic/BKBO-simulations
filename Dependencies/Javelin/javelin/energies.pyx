	...
	cpdef double run(self,
		...
		for neighbor in range(number_of_neighbors):
			...
			e += self.evaluate(a[cell[0], cell[1], cell[2], neighbors[neighbor,0]],
							   ...
							   neighbors[neighbor,0], neighbors[neighbor,1], neighbors[neighbor,2], neighbors[neighbor,3], neighbors[neighbor,4])
		return e
	cpdef double evaluate(self,
						  int a1, double x1, double y1, double z1,
						  int a2, double x2, double y2, double z2,
...
while negative J creates positive correlations"""
	...
	cpdef double evaluate(self,
						  int a1, double x1, double y1, double z1,
						  int a2, double x2, double y2, double z2,
						  Py_ssize_t site1, Py_ssize_t site2, Py_ssize_t target_x, Py_ssize_t target_y, Py_ssize_t target_z) except *:
...
cdef class LennardJonesEnergy(Energy):
	...
	cpdef double evaluate(self,
		...
		return INFINITY if diff == 0 else self.D * (pow(self.desired/diff, 12) - 2*pow(self.desired/diff, 6))
